...
void CollisionCheck_HitSolid(GlobalContext* globalCtx, ColliderInfo* info, Collider* collider, Vec3f* hitPos) {
    ...
    if (flags == TOUCH_SFX_NORMAL && collider->colType != COLTYPE_METAL) {
        ...
        if (collider->actor == NULL) {
            Audio_PlaySfxById1(NA_SE_IT_SHIELD_BOUND);
        } else {
            Audio_PlaySfxByPosAndId2(&collider->actor->projectedPos, NA_SE_IT_SHIELD_BOUND);
        }
        ...
        if (collider->actor == NULL) {
            Audio_PlaySfxById1(NA_SE_IT_REFLECTION_WOOD);
        } else {
            Audio_PlaySfxByPosAndId2(&collider->actor->projectedPos, NA_SE_IT_REFLECTION_WOOD);
        }
    }
}
...
s32 CollisionCheck_SwordHitAudio(Collider* at, ColliderInfo* acInfo) {
    if (at->actor != NULL && at->actor->category == ACTORCAT_PLAYER) {
        if (acInfo->elemType == ELEMTYPE_UNK0) {
            Audio_PlaySfxByPosAndId2(&at->actor->projectedPos, NA_SE_IT_SWORD_STRIKE);
        } else if (acInfo->elemType == ELEMTYPE_UNK1) {
            Audio_PlaySfxByPosAndId2(&at->actor->projectedPos, NA_SE_IT_SWORD_STRIKE_HARD);
        } else if (acInfo->elemType == ELEMTYPE_UNK2) {
            Audio_PlaySfxByPosAndId2(&at->actor->projectedPos, 0);
...
void CollisionCheck_HitEffects(GlobalContext* globalCtx, Collider* at, ColliderInfo* atInfo, Collider* ac,
    ...
    if (ac->actor != NULL) {
        ...
        if (ac->actor == NULL) {
            ...
            Audio_PlaySfxByPosAndId2(&ac->actor->projectedPos, NA_SE_IT_SHIELD_BOUND);
        }
    }
}
...
void CollisionCheck_SpawnShieldParticlesMetal(GlobalContext* globalCtx, Vec3f* v) {
    ...
    Audio_PlaySfxById1(NA_SE_IT_SHIELD_REFLECT_SW);
}
...
void CollisionCheck_SpawnShieldParticlesMetalSound(GlobalContext* globalCtx, Vec3f* v, Vec3f* pos) {
    CollisionCheck_SpawnShieldParticles(globalCtx, v);
    Audio_PlaySfxByPosAndId2(pos, NA_SE_IT_SHIELD_REFLECT_SW);
}
...
void CollisionCheck_SpawnShieldParticlesWood(GlobalContext* globalCtx, Vec3f* v, Vec3f* pos) {
    ...
    Audio_PlaySfxByPosAndId2(pos, NA_SE_IT_REFLECTION_WOOD);
}
