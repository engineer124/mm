...
void Sram_SaveEndOfCycle(PlayState* play) {
    ...
    CLEAR_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_RUPEES);
    CLEAR_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_BOMB_AMMO);
    CLEAR_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_NUT_AMMO);
    CLEAR_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_STICK_AMMO);
    CLEAR_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_ARROW_AMMO);

    if (gSaveContext.save.playerData.rupees != 0) {
        SET_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_RUPEES);
    }

    if (INV_CONTENT(ITEM_BOMB) == ITEM_BOMB) {
        item = INV_CONTENT(ITEM_BOMB);
        if (AMMO(item) != 0) {
            SET_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_BOMB_AMMO);
        }
    }
    if (INV_CONTENT(ITEM_NUT) == ITEM_NUT) {
        item = INV_CONTENT(ITEM_NUT);
        if (AMMO(item) != 0) {
            SET_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_NUT_AMMO);
        }
    }
    if (INV_CONTENT(ITEM_STICK) == ITEM_STICK) {
        item = INV_CONTENT(ITEM_STICK);
        if (AMMO(item) != 0) {
            SET_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_STICK_AMMO);
        }
    }
    if (INV_CONTENT(ITEM_BOW) == ITEM_BOW) {
        item = INV_CONTENT(ITEM_BOW);
        if (AMMO(item) != 0) {
            SET_EVENTINF(EVENTINF_ENDOFCYCLE_HAS_ARROW_AMMO);
        }
    }
