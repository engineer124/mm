...
void AudioLoad_Init(void* heap, size_t heapSize) {
    ...
    for (i = 0; i < ARRAY_COUNT(gAudioCtx.aiBufNumSamples); i++) {
        gAudioCtx.aiBufNumSamples[i] = 10 * SAMPLES_PER_FRAME;
    }
    ...
    permanentPoolAddr = AudioHeap_Alloc(&gAudioCtx.initPool, gAudioHeapInitSizes.permanentPoolSize);
    if (permanentPoolAddr == NULL) {
        // cast away const from gAudioHeapInitSizes
        *((u32*)&gAudioHeapInitSizes.permanentPoolSize) = 0;
    }

    AudioHeap_InitPool(&gAudioCtx.permanentPool, permanentPoolAddr, gAudioHeapInitSizes.permanentPoolSize);
