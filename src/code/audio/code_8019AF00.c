...
void Audio_PlaySeqWithPlayerIO(s8 playerIndex, u16 seqId, u8 fadeTimer, s8 arg3, u8 arg4);
...
void AudioOcarina_PlayControllerInput(u8 isOcarinaSfxSuppressedWhenCancelled) {
    ...
    if ((sOcarinaInputButtonStart == 0) ||
        ...
        if ((sCurOcarinaPitch != OCARINA_PITCH_NONE) && (sPrevOcarinaPitch != sCurOcarinaPitch)) {
            ...
            AudioSfx_AddRequest(NA_SE_OC_OCARINA, &gSfxDefaultPos, 4, &sCurOcarinaBendFreq, &sDefaultOcarinaVolume,
                                &gSfxDefaultReverb);
    ...
    if (sPlaybackNoteTimer == 0) {
        ...
        if (sPlaybackPitch != sPlaybackSong[sPlaybackNotePos].pitch) {
            ...
            if (sPlaybackPitch != OCARINA_PITCH_NONE) {
                ...
                AudioSfx_AddRequest(NA_SE_OC_OCARINA, &gSfxDefaultPos, 4, &sRelativeNotePlaybackBend,
...
void Audio_PlayMainBgm(s8 seqId) {
    AudioThread_QueueCmdS32(0x82 << 24 | SEQ_PLAYER_BGM_MAIN << 16 | (((u8)seqId & 0xFF) << 8), 1);
}
...
f32 AudioSfx_ComputeVolume(u8 bankId, u8 entryIndex) {
    ...
    if (bankEntry->dist > 10000.0f) {
        ...
        if (bankEntry->sfxParams & SFX_PARAM_01_MASK_UPPER) {
            phi_f12 = 0.35f;
            phi_f14 = 0.65f;
        } else {
            phi_f12 = 0.81f;
...
u8 AudioSfx_ComputeSurroundSoundFilter(f32 behindScreenZ, SfxBankEntry* entry, s8 arg2) {
    u8 phi_v1;
    u8 phi_a0;
    u16* sfxParams;
    f32 phi_f0;
    ...
    if (*sfxParams & SFX_FLAG_9) {
        phi_v1 = 0xF;
    }

    switch ((*sfxParams & SFX_PARAM_01_MASK)) {
        case 0:
            phi_f0 = 15.0f;
            break;
...
void AudioSfx_SetProperties(u8 bankId, u8 entryIndex, u8 channelIndex) {
    ...
    switch (bankId) {
        ...
        case BANK_OCARINA:
            ...
            if (((*entry->reverbAdd & 0x80) | baseFilter | sAudioExtraFilter | (entry->sfxFlags & SFX_FLAG2_7)) != 0) {
                filter = ((((*entry->reverbAdd & 0x80) >> 2) | baseFilter | sAudioExtraFilter |
                           ((entry->sfxFlags & SFX_FLAG2_7) >> 2)) *
                          2);
                filter &= 0xFF;
            } else if ((sSoundMode == AUDIO_FS_SURROUND) && !(entry->sfxParams & SFX_FLAG_13)) {
    ...
    AudioThread_QueueCmdS8(6 << 24 | 2 << 16 | (channelIndex << 8) | 2, volS8);
    ...
    if (sSfxChannelState[channelIndex].unk_0D != zVolume) {
        sSfxChannelState[channelIndex].unk_0D = zVolume;
    }
...
u32 func_8019F024(u8 seqScriptValIn, SequenceChannel* channel) {
...
void Audio_PlaySfx(u16 sfxId) {
    AudioSfx_AddRequest(sfxId, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                        &gSfxDefaultReverb);
...
void Audio_PlaySfx_AtPosWithPresetLowFreqAndHighReverb(Vec3f* pos, u16 sfxId) {
    AudioSfx_AddRequest(sfxId, pos, 4, &sTwoSemitonesLoweredFreq, &gSfxDefaultFreqAndVolScale, &sSfxIncreasedReverb);
}

void Audio_PlaySfx_AtPos(Vec3f* pos, u16 sfxId) {
    AudioSfx_AddRequest(sfxId, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
}
...
void Audio_PlaySfx_Underwater(Vec3f* pos, u16 sfxId) {
    if ((sfxId == NA_SE_EN_KONB_JUMP_OLD) || (sfxId == NA_SE_EN_KONB_SINK_OLD)) {
        AudioSfx_AddRequest(sfxId, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                            &gSfxDefaultReverb);
    } else {
        AudioSfx_AddRequest(sfxId, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
...
void Audio_PlaySfx_AtPosForMetalEffectsWithSyncedFreqAndVolume(Vec3f* pos, u16 sfxId, f32 freqVolParam) {
    ...
    AudioSfx_AddRequest(sfxId, pos, 4, &sSfxSyncedFreq, &sSfxSyncedVolume, &gSfxDefaultReverb);
    ...
    if (phi_f0 < freqVolParam) {
        if (phi_v0) {
            ...
            if (metalSfxId != 0) {
                ...
                AudioSfx_AddRequest(metalSfxId, pos, 4, &sSfxSyncedFreq, &sSfxSyncedVolumeForMetalEffects,
                                    &gSfxDefaultReverb);
            }
        }
    }
...
void Audio_PlaySfx_GiantsMaskUnused(Vec3f* pos, u16 sfxId) {
    AudioSfx_AddRequest(sfxId | 0xE0, pos, 4, &sGiantsMaskFreq, &gSfxDefaultFreqAndVolScale, &sGiantsMaskReverbAdd);
}

void Audio_PlaySfx_GiantsMask(Vec3f* pos, u16 sfxId) {
    AudioSfx_AddRequest((sfxId & 0x681F) + 0x20, pos, 4, &sGiantsMaskFreq, &gSfxDefaultFreqAndVolScale,
...
void Audio_PlaySfx_Randomized(Vec3f* pos, u16 baseSfxId, u8 randLim) {
    ...
    AudioSfx_AddRequest(baseSfxId + offset, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                        &gSfxDefaultReverb);
}
...
void Audio_PlaySfx_SwordCharge(Vec3f* pos, u8 chargeLevel) {
    ...
    if (chargeLevel != sPrevChargeLevel) {
        ...
        switch (chargeLevel) {
            case 1:
                AudioSfx_AddRequest(NA_SE_PL_SWORD_CHARGE, pos, 4, &sCurChargeLevelSfxFreq, &gSfxDefaultFreqAndVolScale,
                                    &gSfxDefaultReverb);
                break;
            case 2:
                AudioSfx_AddRequest(NA_SE_PL_SWORD_CHARGE, pos, 4, &sCurChargeLevelSfxFreq, &gSfxDefaultFreqAndVolScale,
    ...
    if (chargeLevel != 0) {
        AudioSfx_AddRequest(NA_SE_IT_SWORD_CHARGE - SFX_FLAG, pos, 4, &sCurChargeLevelSfxFreq,
...
void Audio_PlaySfx_AtPosWithFreqAndVolume(Vec3f* pos, u16 sfxId, f32 freqScale, f32* volume) {
    ...
    if (sfxSettings != NULL) {
        ...
        if (*freqScaleAdj > 0.5f) {
            AudioSfx_AddRequest(sfxId, pos, 4, freqScaleAdj, volume, &gSfxDefaultReverb);
        }
    }
}
...
void Audio_PlaySfx_AtPosWithTimer(Vec3f* pos, u16 sfxId, f32 timerShiftedLerp) {
    sSfxTimer--;
    if (sSfxTimer == 0) {
        AudioSfx_AddRequest(sfxId, pos, 4, &sSfxAdjustedFreq, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
...
void Audio_PlaySfx_AtPosWithReverb(Vec3f* pos, u16 sfxId, s8 reverbAdd) {
    ...
    AudioSfx_AddRequest(sfxId, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &sSfxCustomReverb);
}

void Audio_PlaySfx_AtPosWithVolume(Vec3f* pos, u16 sfxId, f32 volume) {
    gSfxVolume = volume;
    AudioSfx_AddRequest(sfxId, pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxVolume, &gSfxDefaultReverb);
}
...
void Audio_PlaySfx_River(Vec3f* pos, f32 freqScale) {
    ...
    AudioSfx_AddRequest(NA_SE_EV_RIVER_STREAM - SFX_FLAG, pos, 4, &sRiverFreqScaleLerp.value,
                        &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
}
...
void Audio_PlaySfx_Waterfall(Vec3f* pos, f32 freqScale) {
    ...
    AudioSfx_AddRequest(NA_SE_EV_WATER_WALL_BIG - SFX_FLAG, pos, 4, &sWaterfallFreqScaleLerp.value,
...
void Audio_PlaySfx_BigBells(Vec3f* pos, u8 volumeIndex) {
    ...
    AudioSfx_AddRequest(NA_SE_EV_SIGNAL_BIGBELL, pos, 4, &gSfxDefaultFreqAndVolScale, &sBigBellsVolume[volumeIndex & 7],
                        &gSfxDefaultReverb);
}
...
void func_801A0204(s8 seqId) {
    AudioThread_QueueCmdS8(MK_AUDIO_CMD(0x46, 0, 0, 2), (u8)seqId);
}
...
void Audio_PlaySfx_IncreasinglyTransposed(Vec3f* pos, s16 sfxId, u8* semitones) {
    AudioSfx_AddRequest(sfxId, pos, 4, &gPitchFrequencies[semitones[sAudioIncreasingTranspose] + 39],
                        &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
...
void Audio_PlaySfx_Transposed(Vec3f* pos, u16 sfxId, s8 semitone) {
    AudioSfx_AddRequest(sfxId, pos, 4, &gPitchFrequencies[semitone + 39], &gSfxDefaultFreqAndVolScale,
                        &gSfxDefaultReverb);
}
...
void Audio_PlaySfx_AtPosWithAllChannelsIO(Vec3f* pos, u16 sfxId, u8 ioData) {
    ...
    for (i = 0; i < gChannelsPerBank[gSfxChannelLayout][bankId]; i++) {
        AudioThread_QueueCmdS8(_SHIFTL(6, 24, 8) | _SHIFTL(SEQ_PLAYER_SFX, 16, 8) | _SHIFTL(channelIndex++, 8, 8) |
                                   _SHIFTL(6, 0, 8),
                               ioData);
    }
...
void Audio_SplitBgmChannels(s8 volumeSplit) {
    ...
    if ((Audio_GetActiveSequence(SEQ_PLAYER_FANFARE) == NA_BGM_DISABLED) &&
        ...
        for (i = 0; i < 2; i++) {
            ...
            channelBits = 0;
            for (channelIndex = 0; channelIndex < ARRAY_COUNT(gAudioContext.seqPlayers[bgmPlayers[i]].channels);
                 channelIndex++) {
                if (gAudioContext.seqPlayers[bgmPlayers[i]].channels[channelIndex]->notePriority < notePriority) {
...
void Audio_SetSequenceProperties(u8 playerIndex, Vec3f* pos, s16 flags, f32 minDist, f32 maxDist, f32 maxVolume,
                                 f32 minVolume) {
    f32 dist;
    f32 volume;
    s8 sp27;
    ...
    AudioThread_QueueCmdU16(playerIndex << 0x10 | 0x90000000, 0xFFFF);

    if (flags & 1) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x12000000 | 0xFF00, sp27);
    }

    // CHAN_UPD_PAN_SIGNED
    if (flags & 2) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x3000000 | 0xFF00, sp26);
    }

    // CHAN_UPD_PAN_UNSIGNED
    if (flags & 4) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x7000000 | 0xFF00, 0x7F);
    }
    ...
    if (flags & 8) {
        ...
        AudioThread_QueueCmdS32(playerIndex << 0x10 | 0x13000000 | 0xFF00 | 0x54,
                                ((u32)&sSequenceFilter[0] & ~0xF) + 0x10);
    } else {
        // Identity Filter
        // AudioHeap_LoadFilter(((u32)&sSpatialSeqIsActive[SEQ_PLAYER_NATURE] & ~0xF) + 0x10, 0, 0);
        AudioThread_QueueCmdS32(playerIndex << 0x10 | 0x13000000 | 0xFF00, ((u32)&sSequenceFilter[0] & ~0xF) + 0x10);
    }

    if (flags & 0x10) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x14000000 | 0xFF00, 0x7F);
    } else {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x14000000 | 0xFF00, 0);
    }

    // CHAN_UPD_VOL_SCALE
    if (flags & 0x20) {
        AudioThread_QueueCmdF32(playerIndex << 0x10 | 0x1000000 | 0xFF00, maxVolume * volume);
    }

    // CHAN_UPD_PAN_UNSIGNED
    if (flags & 0x40) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x7000000 | 0xFF00, 0x40);
    }

    if (flags & 0x80) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x11000000 | 0xFF00, 1);
    }

    // CHAN_UPD_REVERB
    if (flags & 0x100) {
        AudioThread_QueueCmdS8(playerIndex << 0x10 | 0x5000000 | 0xFF00, 0x37);
    }
}
...
void Audio_UpdateObjSoundSequences(void) {
    if (sObjSoundPlayerIndex != SEQ_PLAYER_INVALID) {
        if ((Audio_GetActiveSequence(sObjSoundPlayerIndex) != NA_BGM_FINAL_HOURS) &&
 ...
 * Used for Main Bgm and Fanfares
 */
void Audio_SetObjSoundProperties(u8 playerIndex, Vec3f* pos, s16 flags, f32 minDist, f32 maxDist, f32 maxVolume,
                                 f32 minVolume) {
    sObjSoundPlayerIndex = playerIndex;
 ...
 *
 */
void Audio_StartFanfareAtPos(u8 playerIndex, Vec3f* pos, s8 seqId, u16 seqIdOffset) {
    s32 pad[3];
    u32 mask;

    if ((Audio_GetActiveSequence(playerIndex) == NA_BGM_FINAL_HOURS) ||
        !Audio_IsSeqCmdNotQueued((playerIndex << 0x18) + NA_BGM_FINAL_HOURS, 0xFF0000FF) || sIsFinalHoursOrSoaring) {
        sIsFinalHoursOrSoaring = true;
    } else if (pos != NULL) {
        if ((seqId != (s8)(Audio_GetActiveSequence(playerIndex) & 0xFFFF)) &&
            !gAudioContext.seqPlayers[playerIndex].enabled && (sObjSoundMainBgmSeqId == NA_BGM_GENERAL_SFX)) {

            mask = 0xFFFF;

            // AudioSeqCmd_StartSequence(playerIndex, ((((AudioThread_NextRandom() % 0x1E) & 0xFF) + 1)), ((seqId &
            // mask) + seqIdOffset));
            Audio_QueueSeqCmd((playerIndex << 0x18) | ((u32)(((AudioThread_NextRandom() % 0x1E) & 0xFF) + 1) << 0x10) |
                              (u32)((seqId & mask) + seqIdOffset));
            sObjSoundMainBgmSeqId = seqId;
        }

        Audio_SetObjSoundProperties(playerIndex, pos, 0x7F, 320.0f, 1280.0f, 1.0f, 0.0f);
    } else {
        AudioSeqCmd_StopSequence(playerIndex, 5);
    }
}
...
void Audio_PlayMainBgmAtPos(Vec3f* pos, s8 seqId) {
    ...
    if (pos != NULL) {
        ...
        if (sObjSoundMainBgmSeqId == NA_BGM_ASTRAL_OBSERVATORY) {
            AudioThread_QueueCmdU16(0x90000000, 0xFFFF);
            AudioThread_QueueCmdF32(0x100FF00, 1.0f);
...
void Audio_StopSequenceAtPos(u8 playerIndex, u8 volumeFadeTimer) {
    AudioSeqCmd_StopSequence((u32)playerIndex, 20);
    if ((playerIndex == SEQ_PLAYER_BGM_SUB) && (Audio_GetActiveSequence(SEQ_PLAYER_BGM_MAIN) != NA_BGM_DISABLED)) {
        Audio_SetVolumeScale(SEQ_PLAYER_BGM_MAIN, 3, 0x7F, volumeFadeTimer);
        Audio_SplitBgmChannels(0);
    }
}

void Audio_StartSubBgmAtPos(u8 playerIndex, Vec3f* pos, u8 seqId, u8 flags, f32 minDist, f32 maxDist, f32 arg6) {
    f32 dist = sqrtf(SQ(pos->z) + ((SQ(pos->x) * 0.25f) + (SQ(pos->y) / 6.0f)));
    u8 targetVolume;
    u16 seqId0 = Audio_GetActiveSequence(playerIndex);
    f32 relVolume;
    s32 pad;

    if (dist > maxDist) {
        if ((u8)seqId0 == seqId) {
            Audio_StopSequenceAtPos(playerIndex, 10);
            sSpatialSeqIsActive[playerIndex] = false;
        }
        return;
    }

    if ((!gAudioContext.seqPlayers[playerIndex].enabled && !sAllPlayersMutedExceptOcaAndSys) ||
        (seqId0 == (NA_BGM_ENEMY | 0x800))) {
        if (playerIndex == SEQ_PLAYER_BGM_SUB) {
            Audio_SetVolumeScale(playerIndex, 3, 0x7F, 1);
        }

        AudioSeqCmd_StartSequence((u32)playerIndex, 1, seqId);

        sSpatialSeqIsActive[playerIndex] = true;
    }

    Audio_SetSequenceProperties(playerIndex, pos, flags, minDist, maxDist, 1.0, 0.05f);

    if ((playerIndex == SEQ_PLAYER_BGM_SUB) && (gAudioContext.seqPlayers[SEQ_PLAYER_BGM_MAIN].enabled == true)) {
 ...
 */
void Audio_PlaySequenceAtDefaultPos(u8 playerIndex, u16 seqId) {
    if (!sAudioCutsceneFlag && (gAudioSpecId != 0xC)) {
        ...
        sSpatialSeqSeqId = seqId;
        sSpatialSeqPlayerIndex = playerIndex;
    }
}
 ...
 */
void Audio_PlaySequenceAtPos(u8 playerIndex, Vec3f* pos, u16 seqId, f32 maxDist) {
 ...
 * Only used in z_demo
 */
void func_801A246C(u8 playerIndex, u8 type) {
    u16 seqId;
    u8 targetVolume;
    s32 channelMask = 0;
    ...
    if (targetVolume != 0xFF) {
        AudioSeqCmd_SetActiveChannels(playerIndex, channelMask);
        Audio_SetVolumeScale(SEQ_PLAYER_BGM_MAIN, 3, targetVolume, 1);
    } else {
        Audio_SetVolumeScale(SEQ_PLAYER_BGM_MAIN, 3, 0x7F, 0);
        seqId = Audio_GetActiveSequence(playerIndex);

        if (seqId != NA_BGM_DISABLED) {
            AudioSeqCmd_StartSequence(playerIndex, 0, seqId);
        }
    }
}
...
void Audio_ScaleTempoAndFreqForSequence(u8 playerIndex, f32 freqTempoScale, u8 duration) {
    if (freqTempoScale == 1.0f) {
        AudioSeqCmd_ResetTempo(playerIndex, duration);
    } else {
        // AudioSeqCmd_ScaleTempo(playerIndex, duration, freqTempoScale)
        Audio_QueueSeqCmd(((u32)(playerIndex << 24)) | 0xB0003000 | (u32)(duration << 0x10) |
                          (u16)(freqTempoScale * 100.0f));
    }

    AudioSeqCmd_SetPlayerFreq(playerIndex, duration, (u16)(freqTempoScale * 1000.0f));
}
...
// Unused
void Audio_SetupNatureAmbienceSequence(u8 natureAmbienceId) {
...
void Audio_PlayFanfareWithPlayerIOPort7(u16 seqId, u8 ioData) {
    Audio_PlayFanfare(seqId);
    AudioSeqCmd_SetPlayerIO(SEQ_PLAYER_FANFARE, 7, ioData);
}
...
/**
 * Sets up seqId to play on playerIndex 1
 */
...
void Audio_PlayFanfareWithPlayerIOCustomPort(u16 seqId, s8 ioPort, u8 ioData) {
    AudioSeqCmd_SetPlayerIO(SEQ_PLAYER_FANFARE, ioPort, ioData);
    Audio_PlayFanfare(seqId);
}

// ======== END FANFARE FUNCTIONS ========

void Audio_PlaySeqWithPlayerIO(s8 playerIndex, u16 seqId, u8 fadeTimer, s8 ioPort, u8 ioData) {
    u16 seqId0;
...
u8 func_801A3950(u8 playerIndex, u8 isChannelIOSet) {
    u8 ret = 0xFF;
    SequenceChannel* channel = gAudioContext.seqPlayers[playerIndex].channels[15];

    if (gAudioContext.seqPlayers[playerIndex].enabled && channel->enabled) {
        if (channel->soundScriptIO[0] != -1) {
            ret = channel->soundScriptIO[0];
            if (isChannelIOSet == true) {
                // Set value 0xFF to channel 15 in io port 0
                AudioSeqCmd_SetChannelIO(playerIndex, 0, 15, 0xFF);
            }
        }
    }
    return ret;
    ...
    if (gAudioContext.seqPlayers[SEQ_PLAYER_BGM_SUB].enabled) {
        if ((Audio_GetActiveSequence(SEQ_PLAYER_BGM_SUB) == NA_BGM_FROG_SONG) && channel->enabled) {
            if (channel->soundScriptIO[0] != -1) {
                ...
                AudioThread_QueueCmdS8(0x6030F00, -1);
            }
        }
    }
    return frogIndex;
...
void Audio_PlaySfx_Window(u8 windowToggleDirection) {
    ...
    if (windowToggleDirection) {
        ...
        // mute all seqplayers
        AudioThread_QueueCmdS32(0xF1 << 24 | 0xFF << 16, 0);
    } else {
        Audio_PlaySfx(NA_SE_SY_WIN_CLOSE);
        // unmute all seqplayers
        AudioThread_QueueCmdS32(0xF2 << 24 | 0xFF << 16, 0);
    }
}
...
void Audio_MuteSeqPlayerBgmSub(u8 isMuted) {
    if (isMuted == true) {
        ...
        AudioThread_QueueCmdS32(0xF1 << 24 | SEQ_PLAYER_BGM_SUB << 16, 0);
    } else {
        // unmute seq player bgm-sub
        AudioThread_QueueCmdS32(0xF2 << 24 | SEQ_PLAYER_BGM_SUB << 16, 0);
    }
}
...
void func_801A3B90(u8 arg0) {
    ...
    if (arg0 != 0) {
        ...
        if (playerIndex == SEQ_PLAYER_BGM_SUB) {
            AudioSeqCmd_SetActiveChannels(playerIndex, channelMask | 3);
        }
    } else {
        if (playerIndex == SEQ_PLAYER_BGM_SUB) {
            Audio_PlaySeqWithPlayerIO(SEQ_PLAYER_BGM_SUB, NA_BGM_ROMANI_RANCH, 0, 0, 0);
...
void Audio_SetBaseFilter(u8 filter) {
    if (sAudioBaseFilter != filter) {
        if (filter == 0) {
            ...
            AudioSfx_AddRequest(NA_SE_PL_IN_BUBBLE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        }
    }
...
void Audio_SetExtraFilter(u8 filter) {
    ...
    if (gActiveSeqs[SEQ_PLAYER_NATURE].seqId == NA_BGM_NATURE_AMBIENCE) {
        for (channelIndex = 0; channelIndex < 16; channelIndex++) {
            ...
            AudioThread_QueueCmdS8((((u32)channelIndex & 0xFF) << 8) | 0x6040000 | 6, filter);
        }
    }
}
...
void Audio_PlaySfx_IfNotInCutsceneImpl(u16 sfxId, Vec3f* pos, u8 token, f32* freqScale, f32* volume, s8* reverbAdd) {
    if (!sAudioCutsceneFlag) {
        AudioSfx_AddRequest(sfxId, pos, token, freqScale, volume, reverbAdd);
    }
}
...
void Audio_MuteAllSeqExceptSysAndOca(u16 duration) {
    ...
    for (channelIndex = 0; channelIndex < 16; channelIndex++) {
        ...
        if (!skip) {
            AudioSeqCmd_SetChannelVol(SEQ_PLAYER_SFX, (u8)(duration >> 1), channelIndex, 0);
        }
    }
}

void Audio_MuteSfxAndNatureSeqExceptSysAndOca(u16 duration) {
    sMuteOnlySfxAndNatureSeq = true;
    Audio_MuteAllSeqExceptSysAndOca(duration);
}

void Audio_StopFanfare(u16 duration) {
    AudioSeqCmd_StopSequence(SEQ_PLAYER_BGM_MAIN, duration);
    if (Audio_GetActiveSequence(SEQ_PLAYER_FANFARE) != NA_BGM_DISABLED) {
        AudioSeqCmd_StopSequence(SEQ_PLAYER_FANFARE, duration);
    }
    AudioSeqCmd_StopSequence(SEQ_PLAYER_BGM_SUB, duration);
    AudioSeqCmd_StopSequence(SEQ_PLAYER_NATURE, duration);
    Audio_SetVolumeScale(SEQ_PLAYER_BGM_MAIN, 3, 0x7F, 0);
    Audio_SetVolumeScale(SEQ_PLAYER_BGM_MAIN, 1, 0x7F, 0);
}
    ...
    if (!sAllPlayersMutedExceptOcaAndSys) {
        for (channelIndex = 0; channelIndex < 16; channelIndex++) {
            switch (channelIndex) {
                ...
                default:
                    AudioSeqCmd_SetChannelVol(SEQ_PLAYER_SFX, 10, channelIndex, volume);
                    break;
            }
        }
    }
...
void Audio_SetSfxReverbIndexExceptOcarinaBank(u8 reverbIndex) {
    ...
    for (channelIndex = 0; channelIndex < 16; channelIndex++) {
        if (channelIndex != SFX_CHANNEL_OCARINA) {
            AudioThread_QueueCmdS8(0x11 << 24 | SEQ_PLAYER_SFX << 16 | (((u32)channelIndex & 0xFF) << 8), reverbIndex);
        }
    }
}
...
void Audio_ResetData(void) {
    ...
    sPrevMainBgmSeqId = NA_BGM_DISABLED;
    AudioThread_QueueCmdS8(0x46000000, -1);
    sRiverSoundBgmPos = NULL;
    sFanfareState = 0;
...
void Audio_SetNatureAmbienceChannelIO(u8 channelIndexRange, u8 ioPort, u8 ioData) {
    u8 firstChannelIdx;
    u8 lastChannelIdx;
    u8 channelIndex;
    ...
    firstChannelIdx = channelIndexRange >> 4;
    lastChannelIdx = channelIndexRange & 0xF;
    if (firstChannelIdx == 0) {
        firstChannelIdx = channelIndexRange & 0xF;
    }

    for (channelIndex = firstChannelIdx; channelIndex <= lastChannelIdx; channelIndex++) {
        AudioSeqCmd_SetChannelIO(SEQ_PLAYER_NATURE, ioPort, channelIndex, ioData);
    }
}
...
void Audio_StartNatureAmbienceSequence(u16 playerIO, u16 channelMask) {
    ...
    AudioSeqCmd_SetPlayerIO(SEQ_PLAYER_NATURE, SEQ_PLAYER_IO_PORT_0, 1);
    AudioSeqCmd_SetPlayerIO(SEQ_PLAYER_NATURE, SEQ_PLAYER_IO_PORT_4, (u8)(playerIO >> 8));
    AudioSeqCmd_SetPlayerIO(SEQ_PLAYER_NATURE, SEQ_PLAYER_IO_PORT_5, (u8)(playerIO & 0xFF));
    ...
    for (channelIndex = 0; channelIndex < 16; channelIndex++) {
        if (!(channelMask & (1 << channelIndex)) && (playerIO & (1 << channelIndex))) {
            AudioSeqCmd_SetChannelIO(SEQ_PLAYER_NATURE, CHANNEL_IO_PORT_1, channelIndex, 1);
        }
    }
}
...
void Audio_PlayNatureAmbienceSequence(u8 natureAmbienceId) {
    ...
    if ((gActiveSeqs[SEQ_PLAYER_NATURE].seqId == NA_BGM_DISABLED) ||
        ...
        Audio_StartNatureAmbienceSequence(sNatureAmbienceData[natureAmbienceId].playerIO,
                                          sNatureAmbienceData[natureAmbienceId].channelMask);

        while ((sNatureAmbienceData[natureAmbienceId].channelIO[i] != 0xFF) &&
               (i < ARRAY_COUNT(sNatureAmbienceData[natureAmbienceId].channelIO))) {
            channelIndex = sNatureAmbienceData[natureAmbienceId].channelIO[i++];
            port = sNatureAmbienceData[natureAmbienceId].channelIO[i++];
            val = sNatureAmbienceData[natureAmbienceId].channelIO[i++];
            AudioSeqCmd_SetChannelIO(SEQ_PLAYER_NATURE, port, channelIndex, val);
        }

        AudioSeqCmd_SetChannelIO(SEQ_PLAYER_NATURE, CHANNEL_IO_PORT_7, NATURE_CHANNEL_UNK, sSoundMode);
    }
}
...
void AudioSfx_Init(u16 fadeTimer) {
    ...
    AudioThread_QueueCmdS32(0xE4000000, func_8019F024);
...
void func_801A4D50(void) {
    AudioSfx_Init(1);
    AudioThread_QueueCmdS32(0xF2FF0000, 1);
    AudioThread_ScheduleProcessCmds();
...
void func_801A4DA4(void) {
    ...
    if (gAudioSpecId < ARRAY_COUNT(gReverbSettingsTable)) {
        ...
        AudioThread_QueueCmdS32(0xE6000100, &gReverbSettingsTable[gAudioSpecId][1]);
    }
}
