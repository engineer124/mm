...
u8 D_801D88B8[] = {
    ...
    0xF3, 0xFC, 0xFF, 0xC3, 0x88, 0x00, 0x29, 0x89, 0x00, 0x2B, 0xDF, 0x7F, 0xE9, 0x0F, 0xDD, 0x37, 0xD4, 0x40, 0xC1,
    0x52, 0xFE, 0x80, 0xF3, 0xFC, 0xFF, 0xC2, 0xFB, 0xC0, 0x00, 0xC1, 0x57, 0xC9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        if (sOcarinaAvailSongs & temp_a3) {
            ...
            if (sp5F) {
                ...
                if (D_801FD4A0[i] >= (sLearnSongExpectedLength[i] - 9)) {
                    ...
                    while (prevNote->noteIdx == note->noteIdx || (note->noteIdx == 0xFF && note->length != 0)) {
...
void func_8019C398(s8 songIdx, u8 arg1) {
    ...
    if (arg1 & 0x80) {
        ...
        phi_a3 = D_801D6FE8;
        for (; i < phi_a3;) {
            if (sOcarinaSongNotes[songIdx - 1][sPlaybackNotePos].noteIdx != 0xFF) {
                i++;
            }
            sPlaybackNotePos++;
        }
    ...
    if ((phi_a3 ^ 0) == 0xFF) {
        while (sPlaybackSong[sPlaybackNotePos].noteIdx == 0xFF && songIdx >= 0x16) {
            sPlaybackNotePos++;
        }
    }
}
...
void func_8019F900(Vec3f* pos, u8 level) {
    ...
    if (level != 0) {
        Audio_PlaySfxGeneral(NA_SE_IT_SWORD_CHARGE - SFX_FLAG, pos, 4, &D_801D6694, &gDefaultSfxFreq,
                             &gDefaultSfxReverb);
    }
}
...
void func_8019FAD8(Vec3f* pos, u16 sfxId, f32 arg2) {
    func_8019FA18(pos, sfxId, arg2, &gDefaultSfxFreq);
}
...
void Audio_PlaySoundRiver(Vec3f* pos, f32 freqScale) {
    ...
    Audio_PlaySfxGeneral(NA_SE_EV_RIVER_STREAM - SFX_FLAG, pos, 4, &sRiverFreqScaleLerp.value, &gDefaultSfxFreq,
                         &gDefaultSfxReverb);
}
...
void Audio_PlaySoundIncreasinglyTransposed(Vec3f* pos, s16 sfxId, u8* semitones) {
    Audio_PlaySfxGeneral(sfxId, pos, 4, &gNoteFrequencies[semitones[sAudioIncreasingTranspose] + 39], &gDefaultSfxFreq,
                         &gDefaultSfxReverb);
...
void Audio_SetBaseFilter(u8 filter) {
    if (sAudioBaseFilter != filter) {
        if (filter == 0) {
            ...
            Audio_PlaySfxGeneral(NA_SE_PL_IN_BUBBLE, &gDefaultSfxPos, 4, &gDefaultSfxFreq, &gDefaultSfxFreq,
                                 &gDefaultSfxReverb);
        }
    }
...
void Audio_PlaySfxByIdIfNotInCutscene(u16 sfxId) {
    Audio_PlaySfxGeneralIfNotInCutscene(sfxId, &gDefaultSfxPos, 4, &gDefaultSfxFreq, &gDefaultSfxFreq,
                                        &gDefaultSfxReverb);
}
