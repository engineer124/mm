...
void AudioPlayback_NoteInitAll(void) {
    ...
    for (i = 0; i < gAudioContext.numNotes; i++) {
        ...
        note->synthesisState.synthesisBuffers = AudioHeap_AllocDmaMemory(&gAudioContext.miscHeap, 0x2E0);
        note->playbackState.attributes.filterBuf = AudioHeap_AllocDmaMemory(&gAudioContext.miscHeap, 0x10);
    }
}
