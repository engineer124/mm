...
s32 Player_InCsMode(PlayState* play) {
    ...
    return Player_InBlockingCsMode(play, player) || (player->attentionMode == PLAYER_UNKAA5_5);
}
...
void func_80124870(PlayState* play, Player* player, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, s32 arg5, s32 arg6,
    ...
    if ((player->stateFlags3 & PLAYER_STATE3_1) || !(player->actor.scale.y >= 0.0f) ||
        (player->stateFlags1 & PLAYER_STATE1_IN_DEATH_CUTSCENE) || (play->unk_18844 != 0)) {
        return;
    }

    sp74 = D_801C0910[player->transformation];
    sp70 = D_801C0924[player->transformation];
    temp_f20 = D_801C0938[player->transformation] - player->shapeOffsetY;
    Matrix_Push();
...
s32 func_80125580(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        sLeftHandType = (s32)player->leftHandType;
        ...
        if ((player->transformation == PLAYER_FORM_ZORA) && (player->stateFlags3 & PLAYER_STATE3_8000)) {
            ...
            Matrix_RotateZS(player->unk_B8E, MTXMODE_APPLY);
            player->upperBodyRot.x = player->unk_B90 - player->unk_B94;
            Matrix_RotateZYX(rot->x, rot->y, rot->z, MTXMODE_APPLY);
            func_80125318(pos, rot);
        ...
        if (limbIndex == PLAYER_LIMB_HEAD) {
            rot->x += player->headRot.z;
            rot->y -= player->headRot.y;
            rot->z += player->headRot.x;
            ...
            if (player->unk_AA8 != 0) {
                Matrix_RotateZS(rot_temp, MTXMODE_APPLY);
                Matrix_RotateYS(player->unk_AA8, MTXMODE_APPLY);
            }

            if (player->upperBodyRot.y != 0) {
                Matrix_RotateYS(player->upperBodyRot.y, MTXMODE_APPLY);
            }

            phi_a0_2 = player->upperBodyRot.x;
            ...
            Matrix_RotateXS(phi_a0_2, MTXMODE_APPLY);
            if (player->upperBodyRot.z != 0) {
                Matrix_RotateZS(player->upperBodyRot.z, MTXMODE_APPLY);
            }
        } else {
...
s32 func_80125D4C(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* actor) {
    ...
    if (!func_80125580(play, limbIndex, dList, pos, rot, player)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            Gfx** dLists = player->leftHandDLists;

            if (player->stateFlags3 & PLAYER_STATE3_2000) {
                rot->z -= player->unk_B8C;
            } else if ((sLeftHandType == 4) && (player->stateFlags1 & PLAYER_STATE1_AWAITING_THROWN_ZORAFINS)) {
                dLists = gPlayerLeftHandOpenDLs + D_801F59E0;
                sLeftHandType = 0;
                ...
                if (phi_v0 != 0) {
                    ...
                    dLists = &D_801C095C[phi_v0][D_801F59E0];
                }
            }
            *dList = *(dLists + D_801F59E4);
            if (player->transformation == PLAYER_FORM_GORON) {
            ...
            if ((player->transformation == PLAYER_FORM_ZORA) &&
                ...
                *dList = gLinkZoraRightHandOpenDL;
                ...
                Gfx** dLists = player->rightHandDLists;
                ...
                if (D_801F59F8 == 8) {
                    if (player->transformation == PLAYER_FORM_HUMAN) {
                        if (player->currentShield != 0) {
                            dLists = &gPlayerHandHoldingShields[2 * ((player->currentShield - 1) ^ 0)];
                        }
                    }
                } else if ((player->rightHandType == 6) && (player->actor.speedXZ > 2.0f) &&
                           (!(player->stateFlags1 & PLAYER_STATE1_SWIMMING))) {
                    dLists = &gPlayerRightHandClosedDLs[D_801F59E0];
                    D_801F59F8 = 7;
                } else {
                    // temp_v0_9 = player->skelAnime.jointTable->unk_84 & 0xF00;
                    phi_v0 = player->skelAnime.jointTable[0x16].x & 0xF00;

                    if (phi_v0 != 0) {
                        phi_v0 = (phi_v0 >> 8) - 1;
                        dLists = &D_801C0964[phi_v0][D_801F59E0];
                    }
                }
                *dList = dLists[D_801F59E4];
            ...
            Gfx** dLists = player->sheathDLists;

            if (player->transformation == PLAYER_FORM_HUMAN) {
                val = GET_CUR_EQUIP_VALUE(EQUIP_TYPE_SWORD);

                if (val != 0) {
                    if ((player->sheathType == 0xE) || (player->sheathType == 0xC)) {
                        dLists = &gPlayerSheathedSwords[2 * ((val - 1) ^ 0)];
                    } else {
                        dLists = &gPlayerSwordSheaths[2 * ((val - 1) ^ 0)];
                    }
                }
            }
            *dList = dLists[D_801F59E4];
...
void func_8012669C(PlayState* play, Player* player, Vec3f* arg2, Vec3f* arg3) {
    ...
    if (player->meleeWeaponState != 0) {
        ...
        if ((player->meleeWeaponState > 0) &&
            ((player->meleeWeaponAnimation < 0x1E) || (player->stateFlags2 & PLAYER_STATE2_RELEASING_SPIN_ATTACK))) {
...
s32 func_80128640(PlayState* play, Player* player, Gfx* dlist) {
    ...
    if ((temp_v1) ||
        ...
        // Draw and jiggle Zora's left hand while playing the guitar
        s16 leftHandRotX =
            Math_SinS(player->unk_B86[0]) * (ABS_ALT(player->upperBodyRot.x) * ((f32)(IREG(52) + 20)) / 100.0f);
        s16 leftHandRotY =
            Math_SinS(player->unk_B86[1]) * (ABS_ALT(player->upperBodyRot.y) * ((f32)(IREG(53) + 15)) / 100.0f);

        OPEN_DISPS(play->state.gfxCtx);

        Matrix_Push();
        Matrix_RotateXS(leftHandRotX, MTXMODE_APPLY);
        Matrix_RotateYS(leftHandRotY, MTXMODE_APPLY);
...
void func_80128BD0(PlayState* play, s32 limbIndex, Gfx** dList1, Gfx** dList2, Vec3s* rot, Actor* actor) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (player->actor.scale.y >= 0.0f) {
            if (player->rightHandType == 0xFF) {
                ...
                if (sp224 != NULL) {
                    ...
                    if (Actor_PlayerIsAimingReadyFpsItem(player) != 0) {
            ...
            if ((player->getItemDrawId != GID_BOTTLE) ||
                ((Actor_PlayerIsAimingFpsItem(player) == 0) && (sp224 != NULL))) {
                if (!(player->stateFlags1 & PLAYER_STATE1_GETTING_ITEM) && (player->getItemDrawId != GID_BOTTLE) &&
        ...
        if ((*dList1 != NULL) && ((player->currentMask != PLAYER_MASK_NONE)) &&
            ...
            if (func_80127438(play, player, player->currentMask)) {
                ...
                if (player->currentMask == PLAYER_MASK_COUPLE) {
                    ...
                    Matrix_Push();
                    Matrix_Translate(temp_s0_6->x, temp_s0_6->z, 0.0f, MTXMODE_APPLY);
        ...
        if ((player->stateFlags1 & (PLAYER_STATE1_100 | PLAYER_STATE1_2)) && (player->genericTimer != 0)) {
            OPEN_DISPS(play->state.gfxCtx);
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gDPSetEnvColor(POLY_XLU_DISP++, 0, 0, 255, (u8)player->genericTimer);
