...
void func_80122F28(Player* player) {
    if ((player->actor.category == ACTORCAT_PLAYER) &&
        !(player->stateFlags1 & (PLAYER_STATE1_GETTING_ITEM | PLAYER_STATE1_CARRYING_ACTOR | PLAYER_STATE1_CLIMBING |
...
bool Player_InBlockingCsMode(PlayState* play, Player* player) {
    return (player->stateFlags1 & (PLAYER_STATE1_DEAD | PLAYER_STATE1_200 | PLAYER_STATE1_IN_CUTSCENE)) ||
...
s32 func_801235DC(PlayState* play, f32 arg1, s16 arg2) {
    ...
    if (player->stateFlags3 & PLAYER_STATE3_GORON_CURLED) {
        player->spinAttackTimer = arg1;
        player->dekuStickLength += arg1 * 0.05f;
        player->yaw = arg2;
...
bool Player_IsBurningStickInRange(PlayState* play, Vec3f* pos, f32 xzRange, f32 yRange) {
    ...
    if ((player->heldItemAction == PLAYER_IA_DEKU_STICK) && (player->stickFlameTimer != 0)) {
...
void Player_AdjustSingleLeg(PlayState* play, Player* player, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot,
    ...
    sp7C = D_801C0938[player->transformation] - player->shapeOffsetY;
    Matrix_Push();
            ...
            *dList = leftHandDLists[sPlayerLod];
            ...
            if (player->transformation == PLAYER_FORM_GORON) {
                ...
                if ((player->stateFlags1 & PLAYER_STATE1_IS_CHANGING_PLAYER_FORM) ||
                    (player->stateFlags1 & PLAYER_STATE1_GETTING_ITEM) || Player_IsFreeSwimming(player)) {
            ...
            if ((player->transformation == PLAYER_FORM_ZORA) &&
                (((player->stateFlags1 & PLAYER_STATE1_IS_CHANGING_PLAYER_FORM)) ||
                 (player->stateFlags1 & PLAYER_STATE1_GETTING_ITEM) || Player_IsFreeSwimming(player))) {
...
Gfx* D_801C0B14[] = {
    object_link_nuts_DL_008760, // PLAYER_HAND_LEFT
    object_link_nuts_DL_008660, // PLAYER_HAND_RIGHT
};

u8 D_801C0B1C[] = {
    PLAYER_BODYPART_LEFT_HAND,  // PLAYER_HAND_LEFT
    PLAYER_BODYPART_RIGHT_HAND, // PLAYER_HAND_RIGHT
};
...
void func_8012669C(PlayState* play, Player* player, Vec3f* arg2, Vec3f* arg3) {
    ...
    if (player->meleeWeaponState != PLAYER_MELEE_WEAPON_STATE_0) {
        if (func_80126440(play, NULL, &player->meleeWeaponInfo[0], &sp3C, &sp30) &&
            (player->transformation != PLAYER_FORM_GORON) && !(player->stateFlags1 & PLAYER_STATE1_HOLDING_SHIELD)) {
...
void Player_DrawGetItem(PlayState* play, Player* player) {
    if (!player->giObjectLoading || (osRecvMesg(&player->giObjectLoadQueue, NULL, OS_MESG_NOBLOCK) == 0)) {
        ...
        if ((player->actor.id == ACTOR_EN_TEST3) ||
            ((player->transformation == PLAYER_FORM_DEKU) && (player->stateFlags1 & PLAYER_STATE1_GETTING_ITEM))) {
...
void Player_AdjustZoraForearms(PlayState* play, Player* player, s32 forearmSide) {
    if ((player->transformation != PLAYER_FORM_ZORA) || (player->rightHandType == PLAYER_MODELTYPE_RH_HOOKSHOT)) {
        return;
    }

    if ((forearmSide != PLAYER_FOREARM_LEFT) && (player->stateFlags1 & PLAYER_STATE1_HOLDING_SHIELD)) {
        // Zora right-arm shielding?
        OPEN_DISPS(play->state.gfxCtx);
        ...
        if (player->stateFlags1 & PLAYER_STATE1_ZORA_BOOMERANG_THROWN) {
            ...
            if ((player->zoraBoomerangActor->params == forearmSide) ||
                (((zoraBoomerangActor->child != NULL)) && (zoraBoomerangActor->child->params == forearmSide))) {
                return;
            }
        }
        ...
        if ((player->skelAnime.animation != &gPlayerAnim_pz_gakkiplay) &&
            ...
            gSPDisplayList(POLY_OPA_DISP++, D_801C0AB4[forearmSide]);

            if (player->meleeWeaponState != PLAYER_MELEE_WEAPON_STATE_0) {
                if ((((player->meleeWeaponAnimation == PLAYER_MWA_ZORA_PUNCH_LEFT)) &&
                     (forearmSide == PLAYER_FOREARM_LEFT)) ||
        ...
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx);
        gSPDisplayList(POLY_OPA_DISP++, D_801C0ABC[forearmSide]);

        Matrix_MultVec3f(&D_801C0AC4[forearmSide], &sp58);
        Matrix_MultVec3f(&D_801C0ADC[forearmSide], &sp4C);

        if (func_80126440(play, NULL, &player->meleeWeaponInfo[forearmSide], &sp58, &sp4C) &&
            (player->stateFlags1 & PLAYER_STATE1_SWIMMING)) {
            EffectBlure_AddVertex(Effect_GetByIndex(player->meleeWeaponEffectIndex[forearmSide]),
                                  &player->meleeWeaponInfo[forearmSide].tip,
                                  &player->meleeWeaponInfo[forearmSide].base);
        }
        Matrix_Pop();
...
s32 Player_DrawDekuFlowers(PlayState* play, Player* player, s32 handSide) {
    if (player->transformation == PLAYER_FORM_DEKU) {
        if (((player->skelAnime.animation == &gPlayerAnim_pn_kakku)) ||
            ...
            gSPDisplayList(POLY_OPA_DISP++, D_801C0B14[handSide]);
            ...
            Matrix_MultZero(&player->bodyPartsPos[D_801C0B1C[handSide]]);
            Matrix_Pop();
...
s32 Player_DrawLeftHandItems(PlayState* play, Player* player, Gfx* dlist) {
    ...
    if (isTakingMaskOff ||
        ...
        (player->stateFlags2 & PLAYER_STATE2_DRAW_MASK_IN_HAND)) {
        s32 mask;
        ...
        Matrix_Scale(1.0f, player->dekuStickLength, 1.0f, MTXMODE_APPLY);
        ...
        s16 sp26 = Math_SinS(player->zoraGuitarLeftHandRot[0]) *
                   (ABS_ALT(player->upperLimbRot.x) * ((f32)(IREG(52) + 20)) / 100.0f);
        s16 sp24 = Math_SinS(player->zoraGuitarLeftHandRot[1]) *
                   (ABS_ALT(player->upperLimbRot.y) * ((f32)(IREG(53) + 15)) / 100.0f);
    ...
    if (limbIndex == PLAYER_LIMB_LEFT_HAND) {
            ...
            if (!Player_IsHoldingHookshot(player) && ((heldActor = player->heldActor) != NULL)) {
                ...
                if ((player->transformation == PLAYER_FORM_FIERCE_DEITY) ||
                    ...
                    if (player->itemAction == PLAYER_IA_DEKU_STICK) {
                        D_801C0994->x = player->dekuStickLength * 5000.0f;
                    } else {
        ...
        if (*dList1 != NULL) {
            if (player->rightHandType == PLAYER_MODELTYPE_RH_BOW) {
                ...
                if ((player->stateFlags3 & PLAYER_STATE3_40) && (player->stickFlameTimer >= 0) &&
                    (player->firstPersonItemTimer < 0xB)) {
                    Vec3f sp20C;
                    f32 temp_fv0;
                    s32 pad2;
                    ...
                    player->spinAttackTimer = temp_fv0 - 3.0f;
                    if (temp_fv0 < 3.0f) {
                        player->spinAttackTimer = 0.0f;
                    } else {
                        player->spinAttackTimer *= 1.6f;
                        if (player->spinAttackTimer > 1.0f) {
                            player->spinAttackTimer = 1.0f;
                        }
                    }
                    player->dekuStickLength = -0.5f;
                }
                Matrix_Scale(1.0f, player->spinAttackTimer, 1.0f, MTXMODE_APPLY);
        ...
        if (player->actor.scale.y >= 0.0f) {
            if (player->rightHandType == PLAYER_MODELTYPE_RH_FF) {
                ...
                if (heldActor != NULL) {
                    ...
                    if (Player_IsAimingFpsItem(player)) {
            ...
            if ((player->getItemDrawIdPlusOne != (GID_NONE + 1)) ||
                ((Player_IsUsingFpsItem(player) == 0) && (heldActor != NULL))) {
                if (!(player->stateFlags1 & PLAYER_STATE1_GETTING_ITEM) &&
        ...
        Player_AdjustZoraForearms(play, player, PLAYER_FOREARM_LEFT);
    } else if (limbIndex == PLAYER_LIMB_RIGHT_FOREARM) {
        Player_AdjustZoraForearms(play, player, PLAYER_FOREARM_RIGHT);
    } else if (limbIndex == PLAYER_LIMB_TORSO) {
        if (player->transformation == PLAYER_FORM_GORON) {
