...
void CutsceneManager_End(void) {
    ...
    switch (sCutsceneMgr.startMethod) {
        case CS_START_2:
            sCutsceneMgr.targetActor->flags &= ~ACTOR_FLAG_UPDATE_DURING_FREEZE;
            FALLTHROUGH;
        case CS_START_1:
...
/**
 * Start an actor cutscene, activate Player Cutscene Action "Wait", turn on ACTOR_FLAG_UPDATE_DURING_FREEZE
 */
