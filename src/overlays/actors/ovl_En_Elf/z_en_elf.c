...
void func_8088E60C(EnElf* this, PlayState* play) {
    ...
    if ((this->unk_244 == 6) || (player->stateFlags1 & PLAYER_STATE1_IS_CHANGING_PLAYER_FORM) ||
        (this->fairyFlags & 0x8000)) {
        glowLightRadius = 0;
    }
        ...
        switch (this->unk_244) {
            ...
            default:
                ...
                if ((player->stateFlags1 & PLAYER_STATE1_TALKING) && (player->talkActor != NULL)) {
                    ...
                    Math_Vec3f_Copy(&nextPos, &play->actorCtx.targetCtx.unk0);
                }
                nextPos.y += 1500.0f * this->actor.scale.y;
...
void func_8088EFA4(EnElf* this, PlayState* play) {
    Actor* arrayPointerActor = play->actorCtx.targetCtx.nextLockOnActor;
    Player* player = GET_PLAYER(play);
    f32 transitionRate;
    ...
    if (this->unk_264 & 8) {
        ...
        if ((arrayPointerActor == NULL) ||
            (Math_Vec3f_DistXYZ(&this->actor.world.pos, &play->actorCtx.targetCtx.unk0) < 50.0f)) {
            this->unk_268 = 1;
        }
    ...
    if (this->fairyFlags & 1) {
        if ((arrayPointerActor == NULL) || (player->lockOnActor == NULL)) {
            this->fairyFlags ^= 1;
        }
    } else if ((arrayPointerActor != NULL) && (player->lockOnActor != NULL)) {
...
void func_8088FA38(EnElf* this, PlayState* play) {
    ...
    if (this->fairyFlags & 0x10) {
        refPos = play->actorCtx.targetCtx.unk0;
