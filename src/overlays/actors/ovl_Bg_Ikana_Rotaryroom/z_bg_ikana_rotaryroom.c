...
void func_80B814B8(BgIkanaRotaryroom* this, PlayState* play) {
    ...
    if (ActorCutscene_GetCurrentIndex() == this->dyna.actor.cutscene) {
        if (player->actor.bgCheckFlags & 0x100) {
            Player_PlaySfx(player, NA_SE_VO_LI_DAMAGE_S + player->ageProperties->voiceSfxOffset);
            Play_TriggerVoidOut(&play->state);
            Player_PlaySfx(player, NA_SE_VO_LI_TAKEN_AWAY + player->ageProperties->voiceSfxOffset);
            play->unk_18845 = 1;
