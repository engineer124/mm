...
void DoorAna_WaitOpen(DoorAna* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.scale.x, 0.01f, 0.001f)) {
        if ((this->actor.targetMode != 0) && (play->transitionTrigger == TRANS_TRIGGER_OFF) &&
            (play->transitionMode == TRANS_MODE_OFF) && (player->stateFlags1 & PLAYER_STATE1_FALLING_INTO_GROTTO) &&
            (player->actionVar8 == 0)) {
            ...
            DoorAna_SetupAction(this, DoorAna_GrabLink);
                   ...
                   !(player->stateFlags1 & (PLAYER_STATE1_SWIMMING | PLAYER_STATE1_RIDING_HORSE)) &&
