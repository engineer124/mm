...
void func_80897A94(EnPeehat* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderTris.base.atFlags & AT_BOUNCED) {
        ...
        SoundSource_PlaySoundByPosition(globalCtx, &this->actor.world.pos, 11, NA_SE_EN_EXTINCT);
        if (!(this->actor.bgCheckFlags & 1)) {
            SoundSource_PlaySoundByPosition(globalCtx, &this->actor.world.pos, 40, NA_SE_EN_PIHAT_SM_DEAD);
        }
...
void func_80898338(EnPeehat* this, GlobalContext* globalCtx) {
    ...
    if (Math_StepToF(&this->actor.speedXZ, 0.0f, 0.5f)) {
        if (this->actor.params != 0) {
            func_800B3030(globalCtx, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, 40, 7, 0);
            SoundSource_PlaySoundByPosition(globalCtx, &this->actor.world.pos, 30, NA_SE_EN_EXTINCT);
            SoundSource_PlaySoundByPosition(globalCtx, &this->actor.world.pos, 40, NA_SE_EN_PIHAT_SM_DEAD);
            Actor_MarkForDeath(&this->actor);
        } else {
            func_80897864(this);
