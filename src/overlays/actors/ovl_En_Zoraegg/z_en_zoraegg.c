...
void EnZoraegg_Init(Actor* thisx, PlayState* play) {
    ...
    switch (ZORAEGG_GET_1F(&this->actor)) {
        case ZORAEGG_1F_00:
            if (Flags_GetSwitch(play, ZORAEGG_GET_FE00(&this->actor))) {
    ...
    switch (ZORAEGG_GET_1F(&this->actor)) {
        ...
        case ZORAEGG_1F_08:
        case ZORAEGG_1F_09:
            this->cueType = cueTypes[(ZORAEGG_GET_1F(&this->actor)) - ZORAEGG_1F_03];
            Animation_PlayOnce(&this->skelAnime, &object_zoraegg_Anim_001E08);
            this->unk_1EC = 1;
            this->unk_1EE = 0;
            this->unk_1EF = 0;
            this->actionFunc = func_80B32B10;
            if (((ZORAEGG_GET_1F(&this->actor)) - ZORAEGG_1F_03) >= func_80B319A8(play)) {
        ...
        case ZORAEGG_1F_0F:
        case ZORAEGG_1F_10:
            this->cueType = cueTypes[(ZORAEGG_GET_1F(&this->actor)) - ZORAEGG_1F_0A];
            this->unk_1EC = 2;
...
Actor* func_80B31CB4(PlayState* play) {
    ...
    while (actor != NULL) {
        if ((actor->id == ACTOR_EN_ZORAEGG) && (ZORAEGG_GET_1F(actor) == ZORAEGG_1F_11) && (actor->home.rot.z == 0)) {
            actor->home.rot.z = 1;
            return actor;
        }
        actor = actor->next;
...
Actor* func_80B31D14(PlayState* play) {
    ...
    while (actor != NULL) {
        if ((actor->id == ACTOR_EN_ZORAEGG) && (ZORAEGG_GET_1F(actor) == ZORAEGG_1F_03)) {
            return actor;
        }
        actor = actor->next;
    }
...
void func_80B320E0(EnZoraegg* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        Flags_SetSwitch(play, ZORAEGG_GET_FE00(&this->actor));
        Actor_Kill(&this->actor);
