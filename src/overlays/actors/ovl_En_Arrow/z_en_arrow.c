    ...
    if (this->actor.parent != NULL) {
        if (this->actor.params == ENARROW_7) {
            if (Math_SmoothStepToF(&this->bubble.unk_144, 16.0f, 0.07f, 1.8f, 0.0f) > 0.5f) {
                Actor_PlaySfx_FlaggedActorPos(&this->actor, NA_SE_PL_DEKUNUTS_BUBLE_BREATH - SFX_FLAG);
                return;
            }
...
void func_8088ACE0(EnArrow* this, PlayState* play) {
    ...
    if (sp50 || (this->unk_262 != 0)) {
        ...
        if (this->actor.params == ENARROW_7) {
            ...
            Actor_PlaySfx_FlaggedActorPos(&this->actor, NA_SE_IT_DEKUNUTS_BUBLE_SHOT_LEVEL - SFX_FLAG);
