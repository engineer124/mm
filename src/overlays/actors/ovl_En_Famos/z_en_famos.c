...
void EnFamos_UpdateBobbingHeight(EnFamos* this) {
    ...
    if (ABS_ALT(this->flipRot) > 0x4000) { // is famos upside down
        Actor_PlaySfx_FlaggedActorPos(&this->actor, NA_SE_EN_FAMOS_FLOAT_REVERSE - SFX_FLAG);
    } else {
        Actor_PlaySfx_FlaggedActorPos(&this->actor, NA_SE_EN_FAMOS_FLOAT - SFX_FLAG);
    }
}
...
void EnFamos_AttackAim(EnFamos* this, PlayState* play) {
    Actor_PlaySfx_FlaggedActorPos(&this->actor, NA_SE_EN_LAST1_FALL_OLD - SFX_FLAG);
