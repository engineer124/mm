...
#define FLAGS (ACTOR_FLAG_NO_UPDATE_CULLING | ACTOR_FLAG_NO_DRAW_CULLING | ACTOR_FLAG_CAN_ATTACH_TO_ARROW)
...
void EnRuppecrow_SpawnRupee(EnRuppecrow* this, PlayState* play) {
    ...
    if (EnRuppecrow_CanSpawnBlueRupees(play) && (this->rupeeIndex % 5) == 4) {
        if (this->rupeeIndex == 19) {
            ...
            rupee = this->rupees[rupeeIndex];
            rupee->unk152 = 60;
            this->rupees[rupeeIndex]->actor.flags |= ACTOR_FLAG_NO_UPDATE_CULLING;
        } else {
            rupee =
