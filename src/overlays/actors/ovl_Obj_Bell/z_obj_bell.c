...
s32 func_80A357A8(ObjBell* this, PlayState* play) {
    ...
    if (this->collider1.base.ocFlags1 & OC1_HIT) {
        ...
        knockbackSpeed = this->unk_21C / 18000.0f;
        if (ABS_ALT(temp_v1) < 0x3FFC) {
            if (this->unk_214 == 0) {
                if (knockbackSpeed > 0.18f) {
                    Player_SetKnockbackLargeNoDamage(play, &this->dyna.actor, 8.0f * knockbackSpeed,
                                                     this->dyna.actor.yawTowardsPlayer, 11.0f * knockbackSpeed);
                    this->unk_214 = 30;
                }
            }
        }
